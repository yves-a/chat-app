/* Lots of help from Tech with Tim */
async function add_messages(msg, scroll) {
    if (typeof msg.name !== "undefined") {
      ...
      var global_name = await find_name();
      var content =
        '<div class="container">' +
        '<b style="color:black" class="right">' +
        msg.name +
        "</b><p>" +
        msg.message +
      ...
      if (global_name == msg.name) {
        content =
          ...
          '<b style="color:black" class="left">' +
          msg.name +
          "</b><p>" +
          msg.message +
...
socket.on("connect", async function () {
  ...
  var form = $("form#msgForm").on("submit", async function (e) {
    ...
    // receive the input from the message box
    let msg_input = document.getElementById("msg");
    let user_input = msg_input.value;
    let user_name = await find_name();

    // after getting the input clear the message box
    msg_input.value = "";
...
window.onload = async function () {
  ...
  /* hide the login if logged in and vice versa */
  let name = await find_name();
  if (name != "") {
    $("#login").hide();
  } else {
    $("#logout").hide();
